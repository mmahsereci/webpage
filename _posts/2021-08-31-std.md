...
In the previous post, we learned that, by the law of large numbers (LLN), the sample mean 
$$\bar{x}_n$$ tends to the population mean $$\mu$$ in some sense for large enough $$n$$ 
(see previous post for notation). 
...
Besides, requiring truly random numbers which (as mentioned in the previous post) are hard to obtain, the SE
